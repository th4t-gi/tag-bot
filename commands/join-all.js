...
module.exports = {
    ...
    interaction.member.guild.members.fetch().then(async members => {
      ...
      await members.filter(m => !m.user.bot).forEach(async m => {
        ...
        //initiate user
        if (await db.get(m.user.username) !== undefined) {
          interaction.channel.send(`${m.user.username} has already been initiated`)
        } else {
          db.set(m.user.username, 0)
          interaction.channel.send(`Initiated user ${m.user.username}`)
        }
      })
    })    
	},
